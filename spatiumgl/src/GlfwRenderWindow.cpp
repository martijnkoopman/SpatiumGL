...
namespace spatiumgl
{
	class GlfwRenderWindow::impl : public RenderWindow
	{
	public:
		// Constructor
		impl(bool debug=false)
			: RenderWindow(debug)
			, m_window(nullptr)
		...
		bool createWindow(int width, int height)
		{
			...
			glfwGetFramebufferSize(m_window, &m_framebufferSize[0], &m_framebufferSize[1]); // m_framebufferSize.data(), m_framebufferSize.data() + 1);
	...
	GlfwRenderWindow::GlfwRenderWindow(bool debug)
		: RenderWindow(debug)
		, m_pimpl(std::make_unique<impl>(debug))
	{
	}
