...
TEST(Transform, translationMatrix)
{
  ...
  const spgl::Vector4 translatedPoint =
    translationMatrix * spgl::Vector4(point, 1);
  ...
  const spgl::Vector4 translatedVector =
    translationMatrix * spgl::Vector4(point, 0);
...
TEST(Transform, translate)
{
  ...
  const spgl::Vector3 worldPoint =
    transform.objectPointToWorldPoint(spgl::Vector3(0, 0, 0));
...
TEST(Transform, cameraLookAt)
{
  ...
  EXPECT_NEAR(
    orthoCamera.transform().back()[0], (target - eye).normalized()[0], 0.00001);
  EXPECT_NEAR(
    orthoCamera.transform().back()[1], (target - eye).normalized()[1], 0.00001);
  EXPECT_NEAR(
    orthoCamera.transform().back()[2], (target - eye).normalized()[2], 0.00001);
}
