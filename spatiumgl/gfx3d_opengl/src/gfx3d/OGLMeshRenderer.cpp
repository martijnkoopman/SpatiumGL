...
namespace spatiumgl {
	namespace gfx3d {
			...
			, m_material(material)
		{
			...
			glBufferData(GL_ARRAY_BUFFER, vertexCount * 3 * sizeof(float), verticesFloat.data(), GL_STATIC_DRAW);
      ...
      glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)nullptr);
		...
		const Mesh* OGLMeshRenderer::mesh() const
		{
			return static_cast<const Mesh*>(m_renderObject);
		}

		void OGLMeshRenderer::render(spatiumgl::gfx3d::Camera* camera, double aspect)
		{
			m_shaderProgram.use();
			...
			{
				int meshColorLocation = glGetUniformLocation(m_shaderProgram.shaderProgamId(), "meshColor");
				glUniform4f(meshColorLocation, (float)m_material.color()[0], (float)m_material.color()[1], (float)m_material.color()[2], (float)m_material.color()[3]);
			}
